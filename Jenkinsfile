pipeline {
  ...
  stages {
    ...
    stage("Run tests") {
      ...
      steps {
        script {
          sh "docker build --pull -f docker/Dockerfile-dev -t love-commander-test  ."
          sh "docker run --env LSST_DDS_PARTITION_PREFIX=ci --env LSST_SITE=ci love-commander-test:latest docker/run-tests.sh"
        }
      }
    }
